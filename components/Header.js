...
const Header = () => {
	...
	// Fetch the number of stargazers of the repository (cropost)
	useEffect(() => {
		const fetchStars = async () => {
			const response = await fetch("https://api.github.com/users/kumartul/repos");
			const repos = await response.json();

			const repo = repos.filter(repo => repo.name === "cropost");
	...
	return (
		<header className="w-full bg-[#fafafa] flex items-center justify-between shadow-sm p-2 dark:bg-[#1a1a1a]">
			...
			{/* Logo */}
			<div>
				<h1 className="font-bold text-md md:text-2xl lg:text-2xl text-gray-700 dark:text-gray-300">Cropost ✂️</h1>
			</div>
			...
			<div className="flex items-center space-x-2">
				...
				{/* Github Card */}
				<a
					href="https://www.github.com/kumartul/cropost"
